...
public class GameController {
	...
	public GameController() {
		...
		do {
					...
					if(GUI.getUserLeftButtonPressed("Hvordan vil de komme ud af f&aelig;ngslet", "Sl&aring; med terningen", "bruge et l&oslash;sladelseskort")){
						...
						for(int i = 0; i<2 ; i++){
							...
							if(roll.isPair()){
								...
								if (i<=2) GUI.showMessage("De slog ikke et par, men har " + (2-i) + "forsøg tilbage");
								else GUI.showMessage("De slog stadig ingen par og skal forsat sidde i fængsel til det er deres tur igen");;
							}
						}
			...
			// Next player's turn
			if(!secondTurn) 	turn.nextTurn();

			else 				secondTurn = false;
